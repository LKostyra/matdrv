...
TEST_F(BackendSoftwareTest, MultiplyMatrix)
{
    FDPtr devFD(new int, FDPtrDelete);
    *devFD = open(MATDRV_DEVICE_PATH.c_str(), O_RDWR | O_NONBLOCK);
    ASSERT_GT(*devFD, -1);
...
TEST_F(BackendSoftwareTest, MultiplyIncorrectMatrix)
{
    FDPtr devFD(new int, FDPtrDelete);
    *devFD = open(MATDRV_DEVICE_PATH.c_str(), O_RDWR | O_NONBLOCK);
    ASSERT_GT(*devFD, -1);
    ...
    // create result matrix - ioctl should return error anyway
    matdrv_matrix_t mat3;
    ASSERT_NE(ioctl(*devFD, MATDRV_IOCTL_GET_RESULT, &mat3), 0);
    ASSERT_EQ(errno, EINVAL);
}

TEST_F(BackendSoftwareTest, MultiplyIncorrectResultMatrix)
{
    FDPtr devFD(new int, FDPtrDelete);
    *devFD = open(MATDRV_DEVICE_PATH.c_str(), O_RDWR | O_NONBLOCK);
    ASSERT_GT(*devFD, -1);
